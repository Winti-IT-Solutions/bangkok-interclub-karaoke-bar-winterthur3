...
import { Mic2, Music2, Martini, MessageCircle, Clock, MapPin, Mail, CreditCard } from 'lucide-react'
...
const images = bkkImages.length ? bkkImages : neonImages
...
export default function Home() {
  ...
  const handleSubmit = (e) => {
    ...
    const subject = encodeURIComponent('Kontaktanfrage – Bangkok Interclub Karaoke Bar Website')
  ...
  return (
    <div className='relative min-h-screen text-white overflow-x-hidden bg-vibe'>
      <header className='relative flex items-center min-h-[90vh]'>
        <div className='w-[92vw] max-w-[1200px] mx-auto py-16'>
          ...
          <motion.p initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.15 }}
            ...
            Willkommen in der <strong>Bangkok Interclub Karaoke Bar</strong> – deiner bunten Neon-Oase in Winti.
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.25 }}
            ...
            <div aria-hidden className='pointer-events-none select-none block md:hidden opacity-30'>
              <div className='absolute left-[6%] top-[18%]'><Mic2 className='w-16 h-16 text-yellow-200 drop-shadow-[0_0_12px_#ffd34d]' /></div>
              <div className='absolute right-[10%] top-[34%]'><Music2 className='w-20 h-20 text-yellow-200 drop-shadow-[0_0_12px_#ffd34d]' /></div>
              <div className='absolute left-[14%] bottom-[10%]'><Martini className='w-16 h-16 text-yellow-200 drop-shadow-[0_0_12px_#ffd34d]' /></div>
            </div>
          </motion.div>

          <div aria-hidden className='pointer-events-none select-none hidden md:block'>
        ...
        <motion.section initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}
          ...
          <div className='mt-4 grid sm:grid-cols-2 gap-3'>
            <div className='rounded-2xl border border-white/10 bg-white/5 p-4'>
              <div className='flex items-center gap-2 font-semibold'><Clock className='w-5 h-5'/> Öffnungszeiten</div>
          ...
          <div className='mt-3 rounded-2xl border border-cyan-300/40 bg-cyan-300/10 p-4 text-cyan-200 shadow-[0_0_10px_#00ffff]'>
            <div className='flex items-center gap-2 font-semibold'><CreditCard className='w-5 h-5'/><span>Zahlung / Payment</span></div>
            <p className='mt-1 text-white/95'><strong>DE:</strong> Kredit‑ oder Debitkarte, TWINT – kein Bargeld.</p>
            <p className='text-white/95'><strong>EN:</strong> Credit card or debit, TWINT — no cash accepted.</p>
          </div>
        </motion.section>
      ...
      <footer className='text-center text-white/80 py-8 text-xs'>
        © 2025 Bangkok Interclub Karaoke Bar, Winterthur ·
        <Link to='/anbieterangaben' className='ml-2 underline opacity-70 hover:opacity-100'>Anbieterangaben & Datenschutz</Link>
      </footer>
    </div>
  )
}
